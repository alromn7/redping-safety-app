...
---
## 📡 Low-Power Gadget Location Tracking (Ping Model)

### Overview
The original live tracking approach for gadget recovery used a continuous high-accuracy position stream plus a 30s persistence timer. This imposed unnecessary battery drain for routine device monitoring. The system now defaults to a low-power "Ping" model, performing on-demand single location acquisitions only when the user requests them.

### Modes
| Mode | Battery Use | When to Use | Data Persisted |
|------|-------------|-------------|----------------|
| Ping Location | Minimal (one GPS fix) | Routine checks, last-known spot | `lastKnownLocation`, `lastPingCode`, `lastPingAt`, updated `pingHistory` |
| Live Tracking | High (continuous stream) | Active recovery while moving | Continuous `lastKnownLocation` updates |

### Metadata Fields
- `lastKnownLocation`: `{ latitude, longitude, timestamp, accuracy, altitude, heading, speed }`
- `lastPingCode`: 6-character human-friendly code (A–Z, 2–9, excludes ambiguous chars) attached to each ping
- `lastPingAt`: ISO timestamp when ping completed
- `pingHistory`: Array (max 10) of recent pings:
  ```json
  {
    "timestamp": "2025-11-23T12:34:56.123Z",
    "code": "K7Z3PQ",
    "lat": 37.4219983,
    "lng": -122.084,
    "accuracy": 8.0
  ...
  ```
- Lost Mode fields retained: `isLostMode`, `lostModeActivatedAt`, `lostModeMessage`, `lostModeContact`

### UX Changes
1. Removed automatic live tracking start on page load.
2. Added "Ping Location" button (disabled while working) generating a code + snapshot.
3. Added status card display: last ping age + code (e.g., `Last ping 3m ago (code H2J7QK)`).
4. Added recent ping list (latest 3) with age + accuracy in the status card.
5. Added info dialog explaining battery trade-offs (`Tracking Mode Info`).
6. Live Tracking remains opt-in via play icon with tooltip: "Start Live Tracking (higher battery use)".

### Rationale
| Concern | Continuous Stream | Ping Model Benefit |
|---------|-------------------|--------------------|
| Battery drain | High (persistent GPS) | Single fix only |
| Background timers | Extra wakeups | None (user-driven) |
| Data noise | Many redundant points | Purposeful snapshots |
| User clarity | Implicit tracking | Explicit action, audit codes |

### Implementation Summary
File: `lib/features/gadgets/presentation/pages/find_my_gadget_page.dart`
Key additions:
```dart
bool _isPinging; String? _lastPingCode; DateTime? _lastPingAt; List<Map<String,dynamic>> _pingHistory;
_pingForLocation() => getCurrentPosition(); generate code; persist; update history (max 10); snackbar.
```
History builder keeps final 10 entries; UI shows top 3 (reverse chronological).

### Operational Guidance
- Prefer pings during routine monitoring; reserve live tracking for active recovery.
- Escalate to Lost Mode before enabling extended live tracking sessions.
- Encourage users to share the latest ping code when coordinating recovery.
- If battery is critically low, fall back to last persisted location instead of forcing a new ping.

### Future Enhancements
- Adaptive accuracy (balanced → high only if last fix older than threshold).
- Background deferred ping scheduling (user-configurable hourly window).
- Secure signed ping codes for verifier cross-check (anti-spoof).
- Aggregated heatmap of historical pings (privacy-aware, opt-in).
