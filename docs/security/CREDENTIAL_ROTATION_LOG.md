...
| Firebase client API key | iOS key resource `99f5430d-f6a9-47e7-bb79-ae3c71019db2` | historical git exposure | pending |  | Firebase iOS app config for project `redping-a2e37` | Follow `docs/security/FIREBASE_CLIENT_KEY_REPLACEMENT_RUNBOOK.md`; verified active SOS iOS Firebase app is `com.romana.redping.sos` (`1:557287609270:ios:e9d9a07f62e910b7832717`), the rewritten repo has been realigned locally to that app, and a fresh local-only plist refresh was verified on 2026-05-06, but runtime validation is currently blocked by an existing iOS CocoaPods ML Kit dependency conflict |
